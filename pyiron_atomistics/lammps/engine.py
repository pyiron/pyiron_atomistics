...
def collect_output_log_helper(file_name, prism):
    ...
    if all(
        ...
        if check_ortho_prism(prism=prism):
    ...
    return df, generic_keys_lst, pressure_dict


def collect_dump_file_helper(file_name, prism, element_lst, structure):
...
def remap_indices_helper(lammps_indices, element_lst, structure):
    """
    ...
    """
    lammps_symbol_order = np.array(element_lst)
...
def check_ortho_prism(prism, rtol=0.0, atol=1e-08):
    """
...
def collect_h5md_file_helper(file_name, cell):
    ...
    return steps, positions, cell, forces
