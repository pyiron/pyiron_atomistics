...
class LammpsPotential:
    ...
    @property
    def model(self):
        return "_and_".join(set(self._df.model))

    @property
    def name(self):
        return "_and_".join(set(self._df.name))

    @property
    def species(self):
        species = set([ss for s in self._df.interacting_species for ss in s])
        preset = set(["___".join(s) for s in self._df.preset_species if len(s) > 0])
        if len(preset) == 0:
            return list(species)
        elif len(preset) > 1:
    ...
    @property
    def filename(self):
        return [f for f in set(self._df.filename) if len(f) > 0]

    @property
    def citations(self):
        return "".join(np.unique([c for c in self._df.citations if len(c) > 0]))

    @property
    def is_scaled(self):
        return "scale" in self._df

    @property
    def pair_style(self):
        if len(set(self._df.pair_style)) == 1:
            return "pair_style " + list(set(self._df.pair_style))[0]
        elif "scale" not in self._df:
            return "pair_style hybrid"
        elif all(self._df.scale == 1):
            return "pair_style hybrid/overlay " + " ".join(list(self._df.pair_style))
        return "pair_style hybrid/scaled " + " ".join([str(ss) for s in self._df[["scale", "pair_style"]].values for ss in s])
    ...
    def pair_coeff(self):
        ...
        if "hybrid" in self.pair_style:
            return [
                ...
                for c in self._df[["interacting_species", "pair_style", "pair_coeff"]].values
            ]
        return [
            " ".join(
                ["pair_coeff"] + convert(c[0]) + [c[1]] + ["\n"]
            )
            for c in self._df[["interacting_species", "pair_coeff"]].values
        ]
    ...
    def get_df(self, default_scale=None):
        if default_scale is None or "scale" in self._df:
    ...
    def __mul__(self, scale_or_potential):
        ...
        new_pot = self.copy()
        new_pot._df['scale'] = scale_or_potential
        return new_pot
...
class EAM(LammpsPotential):
    ...
    def __init__(self, *chemical_elements, name=None, pair_style=None):
        if name is None:
            df = LammpsPotentialFile().find(list(chemical_elements)).iloc[0]
        else:
            df = LammpsPotentialFile().find_by_name(name).iloc[0]
        pair_style = df.Config[0].split()[1].replace('\n', '')
        self._initialize_df(
