...
class QuasiNewton:
    ...
    def get_dx(self, g):
        ...
        M = 0
        for _ in range(1000):
            H = self.hessian+M*multi
            self.dx = -np.einsum('ij,j->i', np.linalg.inv(H), g.flatten()).reshape(-1, 3)
            if np.linalg.norm(self.dx, axis=-1).max()<0.1:
                break
            else:
                if multi==0:
                    E, D = np.linalg.eigh(self.hessian)
                    L = np.sign(E)
                    M = np.einsum('ij,j,lj->il', D, L, D)
                multi += 1.
        return self.dx

    def update_hessian(self, g, threshold=1e-4):
