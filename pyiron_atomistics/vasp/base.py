    ...
    def collect(self, directory=os.getcwd(), sorted_indices=None):
        ...
        if outcar_working:
            ...
            if len(self.outcar.parse_dict["magnetization"]) > 0:
                magnetization = np.array(self.outcar.parse_dict["magnetization"], dtype=object).copy()
                final_magmoms = np.array(self.outcar.parse_dict["final_magmoms"], dtype=object).copy()
