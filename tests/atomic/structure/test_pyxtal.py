...
class TestPyxtal(TestCase):
    ...
    def test_return_value(self):
        ...
        self.assertIsInstance(
            pyxtal(1, species=["Fe"], num_ions=[1]),
            Atoms,
            "returned not an Atoms with scalar arguments",
        )
        self.assertIsInstance(
            pyxtal([1, 2], species=["Fe"], num_ions=[1]),
            StructureStorage,
            "returned not a StructureStorage with multiple groups",
        )
        self.assertIsInstance(
            pyxtal(1, species=["Fe"], num_ions=[1], repeat=5),
            StructureStorage,
            "returned not a StructureStorage with repeat given",
        )
        self.assertEqual(
            pyxtal(1, species=["Fe"], num_ions=[1], repeat=5).number_of_structures,
            5,
