...
class TestContainer(TestWithProject):
    ...
    def setUpClass(cls):
        ...
        cls.structures = [
            cls.project.create.structure.bulk(el).repeat(3) for el in cls.elements
        ]
    ...
    def test_set_array(self):
        ...
        self.assertTrue(
            (self.cont.get_array("symbols", 2) == symbols).all(),
        ...
        for i, structure in enumerate(self.structures):
            ...
            self.assertTrue(
                np.allclose(self.cont.get_array("positions", i), structure.positions),
    ...
    def test_translate_frame(self):
        ...
        for s in self.structures:
            self.assertEqual(
                s,
                self.cont.get_structure(s.get_chemical_formula()),
    ...
    def test_add_structure(self):
        ...
        for i, structure in enumerate(self.structures):
            ...
            self.assertEqual(
                self.cont.get_array("identifier", len(self.structures) + i),
                str(len(self.structures) + i),
                "Default identifier is incorrect.",
            )

        self.assertTrue(
            "cell" in self.cont._per_chunk_arrays,
            "Cells are not saved as per chunk array!",
        )
        try:
            ...
            self.cont.add_structure(
                self.project.create.structure.atoms(
                    symbols=["Fe", "Fe", "Fe"],
            ...
            self.fail(
                "Adding a structure with three atoms should not mess with cell storage!"
            )
    ...
    def test_add_structure_kwargs(self):
        ...
        self.cont.add_structure(
            self.structures[0],
            self.structures[0].get_chemical_formula(),
            energy=E,
            forces=F,
            pressure=P,
            fnord=R[None, :],
        ...
        self.assertEqual(
            self.cont.get_array("energy", self.cont.number_of_structures - 1),
            E,
        ...
        self.assertTrue(
            "fnord" in self.cont._per_chunk_arrays,
    ...
    def test_add_structure_spins(self):
        ...
        self.assertTrue(
            np.allclose(spins, fe_read.spins),
    ...
    def test_hdf(self):
        ...
        self.assertEqual(
            len(self.cont),
            len(cont_read),
            "Container size not matching after reading from HDF.",
        )
        self.assertEqual(
            self.cont.num_chunks,
            cont_read.num_chunks,
            "num_chunks does not match after reading from HDF.",
        )
        self.assertEqual(
            self.cont.num_elements,
            cont_read.num_elements,
        ...
        for s1, s2 in zip(self.cont.iter_structures(), cont_read.iter_structures()):
            self.assertEqual(
                s1,
                s2,
                "Structure from get_structure not matching after reading from HDF.",
            )
        ...
        self.assertEqual(
            len(self.cont),
            len(cont_read),
            "Container size not matching after reading from HDF twice.",
        )
        self.assertEqual(
            self.cont.num_chunks,
            cont_read.num_chunks,
        ...
        for s1, s2 in zip(self.cont.iter_structures(), cont_read.iter_structures()):
            self.assertEqual(
                s1,
                s2,
                "Structure from get_structure not matching after reading from HDF twice.",
            )

        self.assertEqual(
            set(self.cont._per_element_arrays.keys()),
            set(cont_read._per_element_arrays.keys()),
            "per atom arrays read are not the same as written",
        )
        self.assertEqual(
            set(self.cont._per_chunk_arrays.keys()),
        ...
        for n in self.cont._per_element_arrays:
            self.assertTrue(
                (
                    self.cont._per_element_arrays[n] == cont_read._per_element_arrays[n]
                ).all(),
        ...
        for n in self.cont._per_chunk_arrays:
            self.assertTrue(
                (
                    self.cont._per_chunk_arrays[n] == cont_read._per_chunk_arrays[n]
                ).all(),
