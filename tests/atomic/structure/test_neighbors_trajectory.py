...
class TestNeighborsTrajectory(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for n in range(3):
            bulk.positions += np.random.normal(scale=0.1, size=bulk.positions.shape)
    ...
    def test_canonical(self):
        ...
        self.assertEqual(
            traj.indices.shape, (3, 1, 12), "indices from trajectory have wrong shape."
        )
        self.assertEqual(
            traj.distances.shape,
            (3, 1, 12),
            "distances from trajectory have wrong shape.",
        )
        self.assertEqual(
            traj.vecs.shape, (3, 1, 12, 3), "vecs from trajectory have wrong shape."
        )
        for i in range(3):
            neigh = self.canonical.get_structure(i).get_neighbors()
            self.assertTrue(
                np.array_equal(traj.indices[i], neigh.indices),
                "indices from trajectory and get_neighbors not equal",
            )
            self.assertTrue(
                np.array_equal(traj.distances[i], neigh.distances),
                "distances from trajectory and get_neighbors not equal",
            )
            self.assertTrue(
                np.array_equal(traj.vecs[i], neigh.vecs),
    ...
    def test_grandcanonical(self):
        ...
        traj.compute_neighbors()
        self.assertEqual(
            traj.indices.shape, (3, 3, 12), "indices from trajectory have wrong shape."
        )
        self.assertEqual(
            traj.distances.shape,
            (3, 3, 12),
        ...
        self.assertEqual(
            traj.vecs.shape, (3, 3, 12, 3), "vecs from trajectory have wrong shape."
        )
        for i in range(3):
            neigh = self.grandcanonical.get_structure(i).get_neighbors()
            self.assertTrue(
                np.array_equal(traj.indices[i][: i + 1], neigh.indices),
                "indices from trajectory and get_neighbors not equal",
            )
            self.assertTrue(
                np.array_equal(traj.distances[i][: i + 1], neigh.distances),
                "distances from trajectory and get_neighbors not equal",
            )
            self.assertTrue(
                np.array_equal(traj.vecs[i][: i + 1], neigh.vecs),
                "vecs from trajectory and get_neighbors not equal",
            )
