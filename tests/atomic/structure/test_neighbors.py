...
class TestAtoms(unittest.TestCase):
    ...
    def test_get_local_shells_ragged(self):
        ...
        neigh = structure.get_neighbors(
            cutoff_radius=3.5, num_neighbors=None, mode="ragged"
        )
    ...
    def test_get_shells_flattened(self):
        ...
        neigh = structure.get_neighbors(
            cutoff_radius=r, num_neighbors=None, mode="flattened"
        )
    ...
    def test_atom_numbers(self):
        ...
        r = structure.cell[0, 0] * 0.49
        neigh = structure.get_neighbors(
            cutoff_radius=r, num_neighbors=None, mode="filled"
        )
        n = len(structure)
        self.assertEqual(neigh.atom_numbers.sum(), int(n * (n - 1) / 2 * 12))
        neigh = structure.get_neighbors(
            cutoff_radius=r, num_neighbors=None, mode="ragged"
        )
    ...
    def test_norm_order(self):
        a_0 = 2.8
        basis = CrystalStructure(
            "Fe", bravais_basis="bcc", lattice_constants=a_0
        ).repeat(10)
        neigh = basis.get_neighbors(
            num_neighbors=None, norm_order=np.inf, cutoff_radius=a_0 + 0.01
        )
    ...
    def test_steinhardt_parameters(self):
        ...
        neigh = (
            StructureFactory()
            .ase.bulk("Mg", a=1, c=np.sqrt(8 / 3))
            .get_neighbors(num_neighbors=12)
        )
        self.assertAlmostEqual(0, neigh.get_steinhardt_parameter(2)[0])
    ...
    def test_numbers_of_neighbors(self):
        ...
        del basis[0]
        neigh = basis.get_neighbors(
            num_neighbors=None, cutoff_radius=0.45 * basis.cell[0, 0]
        )
