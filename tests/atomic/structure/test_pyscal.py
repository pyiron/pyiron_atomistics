...
class Testpyscal(TestWithCleanProject):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.structure = cls.project.create.structure.bulk("Al", a=4, cubic=True).repeat(
            4
        )
    ...
    def test_steinhardt_parameters_returns(self):
        self.assertEqual(
            2,
            len(pas.get_steinhardt_parameter_structure(self.structure)),
            msg="Expected default return value to be a tuple of qs and cluster indices.",
        )
        self.assertIsInstance(
            pas.get_steinhardt_parameter_structure(self.structure, n_clusters=None),
            np.ndarray,
    ...
    def test_steinhardt_parameters_qs(self):
        ...
        qs, _ = pas.get_steinhardt_parameter_structure(
    ...
    def test_steinhardt_parameters_clustering(self):
        ...
        n_clusters = 3
        _, inds = pas.get_steinhardt_parameter_structure(
...
class Testpyscalatoms(unittest.TestCase):
    ...
    def test_steinhardt_parameters(self):
        ...
        qs, _ = self.al_fcc_4.analyse.pyscal_steinhardt_parameter(
            cutoff=0, n_clusters=2, q=qtest
        )
        for c, q in enumerate(qs):
            self.assertLess(np.abs(np.mean(q) - perfect_vals[qtest[c] - 2]), 1e-3)
    ...
    def test_analyse_pyscal_centro_symmetry(self):
        self.assertTrue(
            all(
                [
                    np.isclose(v, 0.0)
                    for v in self.al_fcc.analyse.pyscal_centro_symmetry(
                        num_neighbors=12
                    )
                ]
            )
        ...
        self.assertTrue(
            all(
                [
                    np.isclose(v, 0.0)
                    for v in self.fe_bcc.analyse.pyscal_centro_symmetry(num_neighbors=8)
                ]
            )
        )
        self.assertTrue(
            all(
                [
                    np.isclose(v, 8.7025)
                    for v in self.ti_hcp.analyse.pyscal_centro_symmetry(
                        num_neighbors=12
                    )
                ]
            )
        ...
        self.assertTrue(
            all(
                [
                    ...
                    for v in self.si_dia.analyse.pyscal_centro_symmetry(num_neighbors=4)
                ]
            )
        )
        self.assertEqual(
            len(self.al_fcc.analyse.pyscal_centro_symmetry()), len(self.al_fcc)
        )
        self.assertEqual(
            len(self.fe_bcc.analyse.pyscal_centro_symmetry()), len(self.fe_bcc)
        )
        self.assertEqual(
            len(self.ti_hcp.analyse.pyscal_centro_symmetry()), len(self.ti_hcp)
        )
        self.assertEqual(
            len(self.si_dia.analyse.pyscal_centro_symmetry()), len(self.si_dia)
        )

    def test_analyse_pyscal_voronoi_volume(self):
        self.assertAlmostEqual(
            np.mean(self.al_fcc.analyse.pyscal_voronoi_volume()), 16.60753125
        )
        self.assertAlmostEqual(
            np.mean(self.fe_bcc.analyse.pyscal_voronoi_volume()), 11.8199515
        )
        self.assertAlmostEqual(
            np.mean(self.ti_hcp.analyse.pyscal_voronoi_volume()), 17.65294557
        )
        self.assertAlmostEqual(
            np.mean(self.si_dia.analyse.pyscal_voronoi_volume()), 20.01287587
        )
        self.assertEqual(
            len(self.al_fcc.analyse.pyscal_voronoi_volume()), len(self.al_fcc)
        )
        self.assertEqual(
            len(self.fe_bcc.analyse.pyscal_voronoi_volume()), len(self.fe_bcc)
        )
        self.assertEqual(
            len(self.ti_hcp.analyse.pyscal_voronoi_volume()), len(self.ti_hcp)
        )
        self.assertEqual(
            len(self.si_dia.analyse.pyscal_voronoi_volume()), len(self.si_dia)
        )
    ...
    def test_analyse_pyscal_cna_adaptive(self):
        ...
        res_dict_total = self.al_fcc.analyse.pyscal_cna_adaptive(
            mode="total", ovito_compatibility=False
        )
        self.assertEqual(
        ...
        res_dict_total = self.fe_bcc.analyse.pyscal_cna_adaptive(
            mode="total", ovito_compatibility=False
        )
        self.assertEqual(
            sum([k in res_dict_total.keys() for k in pyscal_keys]), len(pyscal_keys)
        ...
        res_dict_total = self.ti_hcp.analyse.pyscal_cna_adaptive(
            mode="total", ovito_compatibility=False
        )
        self.assertEqual(
            sum([k in res_dict_total.keys() for k in pyscal_keys]), len(pyscal_keys)
        ...
        res_dict_total = self.si_dia.analyse.pyscal_cna_adaptive(
            mode="total", ovito_compatibility=False
        )
        self.assertEqual(
            sum([k in res_dict_total.keys() for k in pyscal_keys]), len(pyscal_keys)
        ...
        res_numeric = self.al_fcc.analyse.pyscal_cna_adaptive(
            mode="numeric", ovito_compatibility=True
        )
        self.assertEqual(len(res_numeric), len(self.al_fcc))
        self.assertTrue(all([v == 1 for v in res_numeric]))
        res_numeric = self.fe_bcc.analyse.pyscal_cna_adaptive(
    ...
    def test_analyse_pyscal_diamond_structure(self):
        ...
        res_dict_total = self.al_fcc.analyse.pyscal_diamond_structure(
            mode="total", ovito_compatibility=False
        )
        self.assertEqual(
            sum([k in res_dict_total.keys() for k in pyscal_keys]), len(pyscal_keys)
        ...
        res_dict_total = self.fe_bcc.analyse.pyscal_diamond_structure(
            mode="total", ovito_compatibility=False
        )
        self.assertEqual(
            sum([k in res_dict_total.keys() for k in pyscal_keys]), len(pyscal_keys)
        ...
        res_dict_total = self.ti_hcp.analyse.pyscal_diamond_structure(
            mode="total", ovito_compatibility=False
        )
        self.assertEqual(
            sum([k in res_dict_total.keys() for k in pyscal_keys]), len(pyscal_keys)
        ...
        res_dict_total = self.si_dia.analyse.pyscal_diamond_structure(
            mode="total", ovito_compatibility=False
        )
        self.assertEqual(
            sum([k in res_dict_total.keys() for k in pyscal_keys]), len(pyscal_keys)
        ...
        res_numeric = self.al_fcc.analyse.pyscal_diamond_structure(
            mode="numeric", ovito_compatibility=False
        )
        self.assertEqual(len(res_numeric), len(self.al_fcc))
        self.assertTrue(all([v == 0 for v in res_numeric]))
        res_numeric = self.fe_bcc.analyse.pyscal_diamond_structure(
            mode="numeric", ovito_compatibility=False
        )
        self.assertEqual(len(res_numeric), len(self.fe_bcc))
        self.assertTrue(all([v == 0 for v in res_numeric]))
        res_numeric = self.ti_hcp.analyse.pyscal_diamond_structure(
            mode="numeric", ovito_compatibility=False
        )
        self.assertEqual(len(res_numeric), len(self.ti_hcp))
        self.assertTrue(all([v == 0 for v in res_numeric]))
        res_numeric = self.si_dia.analyse.pyscal_diamond_structure(
            mode="numeric", ovito_compatibility=False
        )
        self.assertEqual(len(res_numeric), len(self.si_dia))
        self.assertTrue(all([v == 1 for v in res_numeric]))

        res_str = self.al_fcc.analyse.pyscal_diamond_structure(
            mode="str", ovito_compatibility=False
        )
        self.assertEqual(len(res_str), len(self.al_fcc))
        self.assertTrue(all([v == "others" for v in res_str]))
        res_str = self.fe_bcc.analyse.pyscal_diamond_structure(
            mode="str", ovito_compatibility=False
        )
        self.assertEqual(len(res_str), len(self.fe_bcc))
        self.assertTrue(all([v == "others" for v in res_str]))
        res_str = self.ti_hcp.analyse.pyscal_diamond_structure(
            mode="str", ovito_compatibility=False
        )
        self.assertEqual(len(res_str), len(self.ti_hcp))
        self.assertTrue(all([v == "others" for v in res_str]))
        res_str = self.si_dia.analyse.pyscal_diamond_structure(
            mode="str", ovito_compatibility=False
        )
        self.assertEqual(len(res_str), len(self.si_dia))
