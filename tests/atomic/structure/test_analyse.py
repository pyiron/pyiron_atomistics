...
class TestAtoms(unittest.TestCase):
    def test_get_layers(self):
        ...
        struct = CrystalStructure(
            "Al", lattice_constants=a_0, bravais_basis="fcc"
        ).repeat(10)
        ...
        self.assertEqual(
            np.linalg.norm(
                layers - struct.analyse.get_layers(id_list=struct.select_index("Al"))
            ),
            0,
        )
        self.assertEqual(
            np.linalg.norm(
                ...
                - struct.analyse.get_layers(
                    id_list=struct.select_index("Al"), wrap_atoms=False
                )
            ),
            0,
        )
    ...
    def test_get_layers_other_planes(self):
        structure = CrystalStructure(
            "Fe", bravais_basis="fcc", lattice_constants=3.5
        ).repeat(2)
    ...
    def test_get_layers_with_strain(self):
        structure = CrystalStructure(
            "Fe", bravais_basis="bcc", lattice_constants=2.8
        ).repeat(2)
        ...
        self.assertEqual(
            ...
            structure.analyse.get_layers(
                planes=np.linalg.inv(structure.cell).T
            ).tolist(),
        )
    ...
    def test_get_layers_across_pbc(self):
        ...
        self.assertEqual(
            len(np.unique(layers[structure.analyse.get_layers()[:, 0] == 0, 0])), 1
        )
    ...
    def test_tessellations(self):
        ...
        self.assertAlmostEqual(
            np.linalg.norm(
                structure.find_mic(
                    np.diff(
                        structure.positions[structure.analyse.get_delaunay_neighbors()],
                        axis=-2,
                    )
                ),
                axis=-1,
        ...
        self.assertAlmostEqual(
            np.linalg.norm(
                structure.find_mic(
                    np.diff(
                        structure.positions[structure.analyse.get_voronoi_neighbors()],
                        axis=-2,
                    )
                ),
                axis=-1,
    ...
    def test_cluster_positions(self):
        ...
        self.assertEqual(
            bulk.analyse.cluster_positions(np.zeros((2, 3)), return_labels=True)[
                1
            ].tolist(),
            [0, 0],
        )
