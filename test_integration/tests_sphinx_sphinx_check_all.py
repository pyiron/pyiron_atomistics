...
class TestSphinx(unittest.TestCase):
    ...
    def test_Fe_nonmag(self):
        ...
        job.run()
        self.assertLess(
            np.linalg.norm(job["output/generic/forces"]), 1.0e-4, "Forces wrong"
        )
        self.assertTrue(
            np.allclose(
        ...
        self.assertTrue(
            np.allclose(job.structure.cell, job["output/generic/cells"][-1]),
            "Cells not correctly parsed",
        )
        self.assertFalse(
            "atom_spins" in job["output/generic/dft"].list_nodes(), "spins present"
        )
        ...
        self.assertTrue(
            np.allclose(job.structure.positions, job["output/generic/positions"][0]),
    ...
    def test_Fe_ferro_C(self):
        ...
        job.run()
        self.assertTrue(
            "spx_Al_restart" in list(self.project.job_table().job),
            "restart job not found",
        )
        self.assertAlmostEqual(
    ...
    def test_Al_minimize(self):
        ...
        job.structure = self.project.create.structure.ase.bulk(
            "Al", a=self.a_Al, cubic=True
        )
    ...
    def test_Fe_ferro_constraint(self):
        job = self.project.create_job(
            self.project.job_type.Sphinx, "spx_Fe_ferro_constraint"
        )
        job.structure = self.project.create.structure.ase.bulk("Fe", a=self.a_Fe)
        job.structure.set_initial_magnetic_moments(len(job.structure) * [2])
    ...
    def test_Al_save_memory(self):
        job = self.project.create_job(
            self.project.job_type.Sphinx, "spx_Al_save_memory"
        )
        job.structure = self.project.create.structure.ase.bulk("Al", a=self.a_Al)
    ...
    def test_Al_interactive(self):
        job = self.project.create_job(
            self.project.job_type.Sphinx, "spx_Al_interactive"
        )
        job.structure = self.project.create.structure.ase.bulk("Al", a=self.a_Al)
        job.structure.positions[0, 0] += 0.01
